...
# load raw data
  path <- "~/Desktop/honors-thesis-main/00-data/raw-data/raw-election-data/2021_legislative_council_election/party-candidate-info"
...
# loop in all file names to load data
  for(i in 1:length(data_files)) {                              
    assign(paste0("party_2021_legco_", i),                                   
           read.csv(data_files[i]))
  }
```
...
stacked_raw <-
    # merge all loaded data sets
    do.call("rbind", mget(ls(pattern = "^party_2021_legco_*")))
...
stacked_clean <-
  ...
  select(
    lc_code,
    "candidate_number" = `候選人編號.Candidate.Number`,
    "name_candidate" = `Name.of.Candidate`,
    "party_allegiance" = `Prescribed.Body.1`
  )
```

# get the unique party list

```{r}
uni_party_list_2021_legco <- as.data.frame(
  unique(stacked_clean$party_allegiance)
)
```
...
```{r}
path <- "~/Desktop/honors-thesis-main/00-data/raw-data/raw-election-data/2021_legislative_council_election"

write.csv(uni_party_list_2021_legco, 
          glue("{path}/uni_party_list_2021_legco.csv"),
          row.names = FALSE)

write.csv(stacked_clean, 
          glue("{path}/stacked_candidate_list_2021_legco.csv"),
          row.names = FALSE)
```
