...
# calculate the total number of restaurants andthe average rating in each district
total_df <- 
  joined_restaurants_districts %>%
  group_by(fake_unit_id) %>%
  summarise(
...
avg_ratings <- 
  total_df %>%
  select(fake_unit_id, avg_rating)

# add to the main data frame
df_analysis_w_ratings <-
  df_analysis %>%
  left_join(avg_ratings, by = "fake_unit_id")

write_rds(df_analysis_w_ratings, "/Users/jiayili/Desktop/honors-thesis-main/00-data/cleaned-data/df_analysis_w_ratings.RDS")
```
...
# PREP DATA FOR ALTERNATIVE TREATMENT VARIABLES -------------------------------

```{r}
# raw number of restaurants
number_ideo_restaurants <-
  left_join(count_df, total_df, by = "fake_unit_id") %>%
  pivot_wider(
    id_cols = `fake_unit_id`,
    names_from = `ideo_text`,
    names_prefix = "no_restaurants_",
...
df_analysis_raw_no <-
  joined_districts_shares %>%
  left_join(number_ideo_restaurants, by = "fake_unit_id") 

write_rds(df_analysis_raw_no, "/Users/jiayili/Desktop/honors-thesis-main/00-data/cleaned-data/df_analysis_raw_no.RDS")
```
