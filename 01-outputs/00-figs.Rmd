...
---
Purpose: to clean data and create figures in the background and results sections.
...
plt_trend_turnouts <- 
  ...
  geom_point(stat = "identity", color = "cadetblue", 
             fill = "cadetblue", size = 12.5)  +
...
```{r}
pdf('heatmap_blue.pdf', width = 7, height = 4)
  heatmap_blue 
dev.off()  
```
...
```{r}
pdf('heatmap_small.pdf', width = 12, height = 3.5)
  heatmap
dev.off()  
```
...
df_parallel_with_ci <-
  ...
  summarise(
    mean_prop_votes_demo = mean(prop_votes_demo, na.rm = TRUE),
    mean_prop_votes_est = mean(prop_votes_est, na.rm = TRUE),
    mean_prop_turnouts = mean(sum_joined_dist_total, na.rm = TRUE),
    n = n(), # number of observations
    # standard errors
    se_demo = sd(prop_votes_demo, na.rm = TRUE) / sqrt(n),
    se_est = sd(prop_votes_est, na.rm = TRUE) / sqrt(n),
    se_turnouts = sd(sum_joined_dist_total, na.rm = TRUE) / sqrt(n)
  ) %>%
  # find confidence intervals
  mutate(
    lower_ci_demo = mean_prop_votes_demo - qt(0.975, df=n-1) * se_demo,
    upper_ci_demo = mean_prop_votes_demo + qt(0.975, df=n-1) * se_demo,
    lower_ci_est = mean_prop_votes_est - qt(0.975, df=n-1) * se_est,
    upper_ci_est = mean_prop_votes_est + qt(0.975, df=n-1) * se_est,
    lower_ci_turnouts = mean_prop_turnouts - qt(0.975, df=n-1) * se_turnouts,
    upper_ci_turnouts = mean_prop_turnouts + qt(0.975, df=n-1) * se_turnouts
  )
```
...
```{r}
plt_parallel_trends_votes_demo_ci <-
  df_parallel_with_ci %>%
  ggplot(aes(x = year, y = mean_prop_votes_demo, color = group)) +
  geom_point(size = 3) +
  geom_line(linewidth = 1.5) +
  geom_errorbar(aes(ymin = lower_ci_demo, ymax = upper_ci_demo), width = 0.3) +
  geom_vline(xintercept = 2017, linetype = 2, color = "black") +
  scale_x_continuous(breaks = ticks, labels = tick_labels) +
  ylim(c(0, 0.5)) +
  ylab("Vote shares: pro-democracy") +
...
plt_parallel_trends_votes_est_ci <-
  df_parallel_with_ci %>%
  ggplot(aes(x = year, y = mean_prop_votes_est, color = group)) +
  geom_point(size = 3) +
  geom_line(linewidth = 1.5) +
  geom_errorbar(aes(ymin = lower_ci_est, ymax = upper_ci_est), width = 0.3) +
  geom_vline(xintercept = 2017, linetype = 2, color = "black") +
  scale_x_continuous(breaks = ticks, labels = tick_labels) +
  ylim(c(0, 0.7)) +
  ylab("Vote shares: pro-Beijing") +
...
plt_parallel_trends_turnouts_ci <-
  df_parallel_with_ci %>%
  ggplot(aes(x = year, y = mean_prop_turnouts, color = group)) +
  geom_point(size = 3) +
  geom_line(linewidth = 1.5) +
  geom_errorbar(aes(ymin = lower_ci_turnouts, ymax = upper_ci_turnouts), width = 0.3) +
  geom_vline(xintercept = 2017, linetype = 2, color = "black") +
  scale_x_continuous(breaks = ticks, labels = tick_labels) +
  ylab("Turnouts: total votes cast") +
  # recode the legend
...
# put them together
plt_parallel_trends_ci <-
  plot_grid(
  plot_grid(plt_parallel_trends_votes_demo_ci, plt_parallel_trends_votes_est_ci, nrow = 1, ncol = 2, labels = c('A', 'B')),
  plot_grid(NULL, plt_parallel_trends_turnouts_ci, NULL, nrow = 1, rel_widths = c(0.5, 2, 0.5), labels = c("C")),
  nrow = 2
)

pdf('parallel-trends-ci.pdf', width = 11, height = 10)
plt_parallel_trends_ci
dev.off()
```
...
df_hte3_analysis <-
  readRDS(glue("{path}/df_analysis_w_ratings.RDS"))
```
...
# subgroup 3: ratings
# story here: offering consistently exceptionally good food, customers ignore their political preferences

```{r}
# add a group variable in the data frame: median average rating as the threshold
df_hte3_analysis_added <-
  df_hte3_analysis %>%
  mutate(
    group = 
      case_when(
        avg_rating >= median(avg_rating, na.rm = T) ~ "Above median", 
        ... = avg_rating < median(avg_rating, na.rm = T) ~ "Below median", 
        TRUE ~ NA_character_)
  )
...
hte3_a <- 
  felm(prop_votes_demo ~ prop_Yellow|year + fake_unit_id|0|fake_unit_id, 
       data = df_hte3_analysis_added, subset = group == "Above median")

# below median
hte3_b <- 
  felm(prop_votes_demo ~ prop_Yellow|year + fake_unit_id|0|fake_unit_id, 
       data = df_hte3_analysis_added, subset = group == "Below median")
```
...
out <- data.frame(
  ...
  subgroup = c("Restaurant density (per 10,000 people)", "Restaurant density (per 10,000 people)",
               ...
               "Mean restaurant rating", "Mean restaurant rating"),
  # subgroup names
  ...
  coef = c(coef(hte1_a)["prop_Yellow"], coef(hte1_b)["prop_Yellow"],
           ...
           coef(hte3_a)["prop_Yellow"], coef(hte3_b)["prop_Yellow"]),
  # standard errors
  se = c(sqrt(diag(vcov(hte1_a))["prop_Yellow"]), sqrt(diag(vcov(hte1_b))["prop_Yellow"]),
         ...
         sqrt(diag(vcov(hte3_a))["prop_Yellow"]), sqrt(diag(vcov(hte3_b))["prop_Yellow"])),
  # p-value
  pvalue = c(summary(hte1_a)$coefficients["prop_Yellow", "Pr(>|t|)"],
             ...
             summary(hte3_a)$coefficients["prop_Yellow", "Pr(>|t|)"],
             summary(hte3_b)$coefficients["prop_Yellow", "Pr(>|t|)"]),
  # sample size
  ...
  lower = c(tidy(hte1_a, conf.int = TRUE)$conf.low, tidy(hte1_b, conf.int = TRUE)$conf.low,
            ...
            tidy(hte3_a, conf.int = TRUE)$conf.low, tidy(hte3_b, conf.int = TRUE)$conf.low),
  ...
  upper = c(tidy(hte1_a, conf.int = TRUE)$conf.high, tidy(hte1_b, conf.int = TRUE)$conf.high,
            ...
            tidy(hte3_a, conf.int = TRUE)$conf.high, tidy(hte3_b, conf.int = TRUE)$conf.high)
)
...
plt_hte <-
  ...
  facet_wrap(~ subgroup, scales = "free_y", nrow = 3) +
