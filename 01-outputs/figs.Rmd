...
```{r}
# coef_table_hte1 <- data.frame(
#   subgroup = "Restaurant density (per 10,000 people)",
#   
#   group = c("Below Median", "Above Median"),
#   
#   coef = c(coef(hte1)["prop_Yellow:groupBelow Median"],
#            coef(hte1)["prop_Yellow"] + coef(hte1)["prop_Yellow:groupBelow Median"]),
#   
#   se = c(sqrt(diag(vcov(hte1))["prop_Yellow:groupBelow Median"]),
#          sqrt(diag(vcov(hte1))["prop_Yellow"] + diag(vcov(hte1))["prop_Yellow:groupBelow Median"])),
#   
#   pvalue = c(summary(hte1)$coefficients["prop_Yellow:groupBelow Median", "Pr(>|t|)"],
#              summary(hte1)$coefficients["prop_Yellow", "Pr(>|t|)"]),
#   
#   N = c(table(df_hte1_analysis$group)[1], table(df_hte1_analysis$group)[2]),
#   
#   lower = c(coef(hte1)["prop_Yellow:groupBelow Median"] - 1.96*sqrt(diag(vcov(hte1))["prop_Yellow:groupBelow Median"]),
#             (coef(hte1)["prop_Yellow"] + coef(hte1)["prop_Yellow:groupBelow Median"]) - 1.96*sqrt(diag(vcov(hte1))["prop_Yellow"] + diag(vcov(hte1))["prop_Yellow:groupBelow Median"])),
#   
#   upper = c(coef(hte1)["prop_Yellow:groupBelow Median"] + 1.96*sqrt(diag(vcov(hte1))["prop_Yellow:groupBelow Median"]),
#             (coef(hte1)["prop_Yellow"] + coef(hte1)["prop_Yellow:groupBelow Median"]) + 1.96*sqrt(diag(vcov(hte1))["prop_Yellow"] + diag(vcov(hte1))["prop_Yellow:groupBelow Median"]))
# )
```
...
# get coef table 
# hte2 <- 
#   felm(prop_votes_demo ~ prop_Yellow*group|year + fake_unit_id|0|fake_unit_id, 
#        data = df_hte2_analysis)
# df_hte2_analysis$group <- relevel(factor(df_hte2_analysis$group), ref = "Yes")
# coef_table_hte2 <- data.frame(
#   subgroup = "Historically pro-democracy district",
#   
#   group = c("Yes", "No"),
#   
#   coef = c(coef(hte2)["prop_Yellow:groupYes"],
#            coef(hte2)["prop_Yellow"] + coef(hte2)["prop_Yellow:groupYes"]),
#   
#   se = c(sqrt(diag(vcov(hte2))["prop_Yellow:groupYes"]),
#          0.08727),
#   
#   pvalue = c(summary(hte2)$coefficients["prop_Yellow:groupYes", "Pr(>|t|)"],
#              0.000442),
#   
#   N = c(table(df_hte2_analysis$group)[2], table(df_hte2_analysis$group)[1]),
#   
#   lower = c(coef(hte2)["prop_Yellow:groupYes"] - 1.96*sqrt(diag(vcov(hte2))["prop_Yellow:groupYes"]),
#             (coef(hte2)["prop_Yellow"] + coef(hte2)["prop_Yellow:groupYes"]) - 1.96*sqrt(diag(vcov(hte2))["prop_Yellow"] + diag(vcov(hte2))["prop_Yellow:groupYes"])),
#   
#   upper = c(coef(hte2)["prop_Yellow:groupYes"] + 1.96*sqrt(diag(vcov(hte2))["prop_Yellow:groupYes"]),
#             (coef(hte2)["prop_Yellow"] + coef(hte2)["prop_Yellow:groupYes"]) + 1.96*sqrt(diag(vcov(hte2))["prop_Yellow"] + diag(vcov(hte2))["prop_Yellow:groupYes"]))
# )
